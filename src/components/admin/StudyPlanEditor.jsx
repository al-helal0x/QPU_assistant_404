...
export function mockBuildStudyPlanUpdate(courses) {
  return {
    __mock: true,
...
export default function StudyPlanEditor() {
  ...
  const pkg = changed ? mockBuildStudyPlanUpdate(courses) : null;
        ...
        <div className="rounded-lg border border-border bg-bg-subtle p-4">
          {pkg?.__mock ? (
            <div className="rounded-md border border-warning-border bg-warning-bg p-3 text-sm text-warning-text">
              معاينة فقط — النشر الفعلي بانتظار تسليم عضو 5 (<code>buildStudyPlanUpdate</code>).
              التغييرات التالية جاهزة ولن تُفقَد، لكن لا يوجد بعد زر نشر حقيقي يكتبها على GitHub.
            </div>
          ) : (
