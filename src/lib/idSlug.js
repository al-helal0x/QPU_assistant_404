 ...
 *
 * ⚠️ 2026-07-22: العقد الجديد بالواجهة (SubjectForm.jsx) يطلب من الآدمن
 * كتابة الاسم فقط بلا لقب (لأن اللقب "دكتور المادة: " يُضاف تلقائياً عند
 * العرض — انظر Subject.jsx). هذي الدالة تبقى تُزيل أي لقب شائع ("د.",
 * "الدكتور", "الدكتورة", "دكتور", "دكتورة") كطبقة أمان إضافية فقط، لو كتبه
 * الآدمن بالغلط رغم الإرشاد — لا اعتماد وحيد على انضباط الإدخال بالواجهة.
 *
 * لا تكتب فوق دكتور موجود بصمت أبداً — التصادم دائماً يُحَل بلاحقة، لا
 * باستبدال المعرّف.
 */
const PROFESSOR_TITLE_PATTERN = /^(د\.|الدكتورة|الدكتور|دكتورة|دكتور)\s*/;

export function suggestProfessorId(professorName, existingIds = []) {
  const cleaned = (professorName || "").replace(PROFESSOR_TITLE_PATTERN, "").trim();
  const base = transliterateToSlug(cleaned || professorName);
  if (!base) return "";
  const prefixed = base.startsWith("prof-") ? base : `prof-${base}`;
  if (!existingIds.includes(prefixed)) return prefixed;

  let n = 2;
  while (existingIds.includes(`${prefixed}-${n}`)) {
    n += 1;
  }
  return `${prefixed}-${n}`;
}
